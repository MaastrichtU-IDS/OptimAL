...
def getLabels(code, path, output):
    ...
    for filename in os.listdir(path):
        if filename.endswith('.xml'):
            ...
            try:
                ...
                outfile.write(indications[0]+'\t'+indications[1]+'\n')
            except:
    ...
    #ind = pd.DataFrame(all_indications, columns=['Label_ID','Set_ID']) 
    #print ('number of the samples',len(ind))           
...
if __name__ =="__main__":
    ...
    parser.add_argument('-c', required=False, dest='code', help='enter the code from which type of label you want ("34084-4" for adverse reactions and "34067-9" for indication "34070-3" for Contraindications) ' )
    ...
    #print (indications)
    ind = getLabels(code, path, output)
